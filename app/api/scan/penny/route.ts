...
export async function GET() {
  ...
  try {
    const [results, rejected, recentRuns, { remaining }] = await Promise.all([
      pennyStockScanService.getLatestResults(30),
      pennyStockScanService.getLatestRejected(),
      pennyStockScanService.getRecentRunIds(5),
      pennyStockScanService.checkDailyLimit(),
    ]);
    return NextResponse.json({ results, rejected, recentRuns, scansRemainingToday: remaining });
  } catch (err) {
